...
class ilExternalContentFunctions
{
    ...
    private static function signOAuth($a_params)
    {
        ...
        $token = new OAuthToken($a_params["key"], $a_params["secret"]);
        $request = OAuthRequest::from_consumer_and_token($consumer, $token, $a_params["http_method"], $a_params["url"], $a_params["data"]);
